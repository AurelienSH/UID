...
def createData(data):
    ...
    with open(f'../modeling/gpt_gum_nofrac.csv', 'w', newline='') as output_file:
...
#    uid = m.uid_2010(full_conll2)
    ...
    folder = '../Data/'
    full_conll2 = c2.read_conll(f"{folder}concatSUD.conllu",f"{folder}concatUD.conllu")
    genres = c2.get_genres(full_conll2)
    genres2id = {g:i for i, g in zip(range(len(genres)),genres)}
    id2genres = {i:g for g, i in genres2id.items()}
    freq, abs_freq = m.lemma_freq(full_conll2)
    ...
    uid_df = pd.read_csv("scores_gpt_gum_nofrac.tsv",sep="\t", index_col=0)
    ...
    explore_extrema(dataDict, couple_scores,"extrema_exploration_gpt_gum.tsv")
